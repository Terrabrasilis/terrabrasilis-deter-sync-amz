...
curl -k $AUTH $GEOSERVER_BASE_URL'/'$GEOSERVER_BASE_PATH'/'$PROJECT_NAME'/wfs?SERVICE=WFS&REQUEST=GetFeature&VERSION=2.0.0&TYPENAME='$LAYER_NAME'&OUTPUTFORMAT=application%2Fjson' \
...
then
	...
	then
		...
		/usr/local/scripts-shell/updateDate.sh "$TARGET_DIR/$LAYER_NAME.json"
	else
